          ...
          >
            ...
            { this.props.video && (
              <div className={`transform transition ease-in-out duration-500 absolute top-0 left-0 right-0 bottom-0 ${this.props.videoAlwaysOn ? 'opacity-100' : 'hidden md:block opacity-0 group-hover:opacity-100' }`}>
                <video loop={true} autoPlay={this.props.videoAlwaysOn ? 'autoplay' : false } playsInline={this.props.videoAlwaysOn ? true : false} muted preload="meta" className="w-full h-full object-cover z-10" ref={videoRef => this.videoRef = videoRef}>
